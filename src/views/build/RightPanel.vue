  ...
  <div>
    <a-tabs default-active-key="1" >
      <a-tab-pane key="1" tab="组件属性">
        <div class="form-y">
          <a-form-model :model="activeData" :label-col="labelCol" :wrapper-col="wrapperCol">
            ...
            <a-form-item v-if="activeData.labelSpan!==undefined" label="标签宽度">
              <a-input v-model.number="activeData.labelSpan" type="number" placeholder="请输入标签宽度" />
            </a-form-item>
            ...
            <a-form-item v-if="activeData.fileSize !== undefined" label="文件大小">
              <a-input v-model.number="activeData.fileSize" placeholder="请输入文件大小" :style="{ width: '70%' }">
              </a-input>
            ...
            <a-form-item v-if="activeData.tag === 'input'&&activeData.type === 'number'" label="严格步数">
              <a-switch v-model="activeData['step-strictly']" />
            </a-form-item>
            <a-form-item v-if="activeData.type === 'mutil-column-auto'" label="是否多选">
              <a-switch v-model="activeData.props.props.multiple" />
            </a-form-item>
            <a-form-item v-if="activeData.type === 'mutil-column-auto'" label="展示全路径">
              <a-switch v-model="activeData['show-all-levels']" />
            </a-form-item>
            <a-form-item v-if="activeData.type === 'mutil-column-auto'" label="可否筛选">
            ...
            <a-form-item v-if="activeData.type === 'single-column'" label="是否可搜索">
              <a-switch v-model="activeData.filterable" />
            </a-form-item>
            <a-form-item v-if="activeData.type === 'single-column'" label="是否多选">
            ...
            <template
              v-if="['checkbox', 'radio', 'single-column'].indexOf(activeData.type) > -1"
            >
            ...
            <template v-if="activeData.type==='mutil-column-auto'">
              ...
              <div style="position: relative" v-if="activeData.dataType==='static'">
                鼠标右键可设置数据
                <a-tree
                ...
                <div :style="tmpStyle" v-if="NodeTreeItem">
                  ...
                  <div class="menu-item" @click="orgHandle('del')" v-if="NodeTreeItem.parentOrgId">
            ...
            <template v-if="activeData.layout === 'colFormItem'">
              ...
              <div style="margin-left: 20px">
                <a-button type="link" icon="plus" @click="addReg">添加规则</a-button>
              </div>
            </template>
          </a-form-model>
        </div>
      ...
      <a-tab-pane key="2" tab="表单属性" force-render>
        <a-form-model :model="formConf" :label-col="labelCol" :wrapper-col="wrapperCol">
          ...
          <a-form-model-item label="校验模型">
            <a-input v-model="formConf.formRules" disabled placeholder="请输入校验模型" />
          </a-form-model-item>
          ...
          <a-form-model-item label="标签宽度">
            <a-input-number v-model="formConf.labelSpan" placeholder="标签宽度" />
          </a-form-model-item>
    ...
    >
      <a-form v-if="selectDataVisible" :form="selectDataForm" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }" >
        <a-form-item label="选项名">
          <a-input
            v-decorator="['label', { rules: [{ required: true, message: '请输入' }] }]"
          />
        </a-form-item>
        <a-form-item label="选项值">
          <a-input-group compact>
            <a-input
              style="width: 70%;"
              v-decorator="['value', { rules: [{ required: true, message: '请输入' }] }]"
            />
            <a-select
              style="width: 30%;"
              v-decorator="[
    ...
    methods: {
        ...
        this.selectDataForm.validateFields((err, values) => {
          if (!err) {
            if (this.NodeTreeItem) {
              this.nodeHandleAdd(this.activeData.options, this.NodeTreeItem.id, values)
            } else {
      ...
      orgHandle (type) {
        if (type !== 'del') {
          ...
          this.nodeHandleAdd(this.activeData.options, this.NodeTreeItem.id, { type: 'del' })
          this.NodeTreeItem = null
        }
      },
      addTreeItem() {
